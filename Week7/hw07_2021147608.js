...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            cube.updateNormals();
            shadingMode = 'SMOOTH';
            updateText(textOverlay3, "shading mode: " + shadingMode + " (" + mode + ")");
            render();
        }
        else if (event.key == 'f') {
...
async function initShaderGouraud() {
...
async function initShaderPhong() {
...
async function main() {
    try {
        ...
        // creating shaders
        shaderGouraud = await initShaderGouraud();
        shaderPhong = await initShaderPhong();
        ...
        textOverlay3 = setupText(canvas, "shading mode: " + shadingMode + " (" + mode + ")", 3);
