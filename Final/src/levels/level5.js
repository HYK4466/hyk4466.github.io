...
export function initLevel(scene, world) {
    ...
    const obstacle1 = new StaticSimpleCube(scene, world, 30, 1, 1, 0xff0000, 5, 1, -9);
    const obstacle2 = new StaticSimpleCube(scene, world, 30, 1, 1, 0xff0000, -5, 1, 9);

    const obstacle3 = new StaticSimpleCube(scene, world, 0.5, 20, 20, 0xff0000, -10, 10, 0);

    obstacle1.collider.setSensor(true);  // 충돌 판정만, 물리 반응 없음
    obstacle2.collider.setSensor(true);
    obstacle3.collider.setSensor(true);

    obstacleBlocks = [obstacle1, obstacle2, obstacle3];    
    ...
    obstacle12texture(obstacle1, obstacle2);
    obstacle3texture(obstacle3);
...
function obstacle12texture(obstacle1, obstacle2) {
    const textureLoader = new THREE.TextureLoader();
        ...
        map.repeat.set(1, 0.1);
        const normal = textureLoader.load('../../texture/level2/normal.png');
        normal.wrapS = THREE.RepeatWrapping;
        normal.wrapT = THREE.RepeatWrapping;
        normal.repeat.set(1,0.1);
        const rough = textureLoader.load('../../texture/level2/rough.png');
        rough.wrapS = THREE.RepeatWrapping;
        rough.wrapT = THREE.RepeatWrapping;
        rough.repeat.set(1,0.1);
        const bump = textureLoader.load('../../texture/level2/bump.png');
        bump.wrapS = THREE.RepeatWrapping;
        bump.wrapT = THREE.RepeatWrapping;
        bump.repeat.set(1,0.1);
    
        obstacle1.updateMaterialBMap(ao, map, normal, rough, bump);
        obstacle2.updateMaterialBMap(ao, map, normal, rough, bump);
}

function obstacle3texture(obstacle3) {
    const textureLoader = new THREE.TextureLoader();
        ...
        obstacle3.updateMaterialEmissiveMap(ao, map, normal, rough, bump, emissive);
}
...
export function runLoop() {
    ...
    obstacleBlocks[0].updateMeshPosition(5, 1, -9*Math.sin(time));
    obstacleBlocks[1].updateMeshPosition(5, 1, 9*Math.sin(time));

    obstacleBlocks[2].updateMeshPosition(-8*Math.cos(time/2));
}
